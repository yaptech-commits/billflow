...
type NavigationRole = StaffRole | "parent";

function getNav(role: NavigationRole | null) {
  if (role === "parent") {
    return [
      {
        label: "Parent Portal",
        items: [
          { href: "/school/portal", icon: GraduationCap, label: "My Students" },
          { href: "/school/portal#notifications", icon: Bell, label: "Notifications" },
        ],
      },
    ];
  }
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active ? "bg-gold/10 text-gold" : "text-muted hover:bg-border hover:text-surface",
                    )}
                  >
                    <Icon size={17} />
                    {label}
      ...
      <div className="px-4 py-4 border-t border-border flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gold">
            {role === "salesperson" ? "Salesperson" : role === "parent" ? "Parent / Guardian" : role === "superadmin" ? "Super Admin" : "Owner"}
          </p>
        </div>
